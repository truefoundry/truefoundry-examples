...
1. Change working directory to train folder
    ...
    cd train
    ```
2. Create workspace and API key on the TrueFoundry platform with these steps
...
5. To deploy using python script:
   ...
   ```
   To deploy using CLI replace the workspace_fqn in the ``train_deploy.yaml`` file and:
   ```commandline
...
5. <details><summary>Find the model_version_fqn of the model that you want to deploy from</summary>
